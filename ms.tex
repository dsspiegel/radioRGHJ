...
We note that \revise{}{\citet{jardine2008} modeled the reconnection between the magnetic fields of close-in planets and that of the star to obtain estimates which is not based on the ``radiometric Bode's law''. } 
...
\revise{}{Additional two major updates from \citet{ignace2010} are the following. } 
\revise{We also consider}{Firstly, we estimate} the plasma frequency cut-off of the stellar wind, which turns out to be one of the major obstacles to detect radio emission from RGHJs. 
...
\revise{}{We note that the outflow from heated planets are negligible compared to these pressures, as discussed in Appendix \ref{ap:outflow}. %
}
Thus, 
%%%
\begin{equation}
...
\end{eqnarray}
%%%%%%%%%
\revise{}{The accretion luminosity $L_{\rm acc}$ and the temperature $T_{\rm acc}$ are}
%%%%%%%%%
\begin{eqnarray}
...
The detailed electromagnetic structure around RGHJs therefore requires an elaborate numerical simulations, which is beyond the scope of this paper. Noting that all other uncertainties including the radio wave generation, in this paper we simply aim at giving an order-of-magnitude estimate. }
...
\revise{}{We also note that the orbital velocity of the canonical hot Jupiters has been ignored in equation (\ref{eq:P_HJ}); the orbital velocity at $0.05 {\rm AU}$ around a Solar-mass star is $\sim 130~{\rm km/sec}$, which contributes to only $\sim $5\% increase of the velocity. }
...
\revise{}{We note that there data do not unambiguously distinguish RG from early AGB stars and thus the list likely includes about 40\% of early AGB stars. }
...
\revise{}{A more relistic expected number of detection can be also evaluated by combining these results with the probability for a star to have a planet with certain mass and orbit,  %
Observations of main-sequence stars implies that the number of Jovian planets is about a few to ten percent of the number of stars \citep[e.g.][]{cumming2008,cassan2012}. %
Assuming the same population for RG systems, the expected number of actual detection from RGHJs would be order of unity.  
...
A extra fruit from observations of RGHJs in particular is that the lower cut-off frequency, which presumably corresponds to in situ plasma frequency, could probe the stellar wind property at the planetary orbit. In addition, the planetary radio power in such an extreme environment as RG's massive stellar wind will be valuable data points to test the empirical ``radiometric Bode's law''. %
...
\end{eqnarray}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
where $\sigma $ is the cross section of hydrogen atoms, assumed to be equal to Bohr radius here [[OK??]]. 
