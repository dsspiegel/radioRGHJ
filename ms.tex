...
%%%%%%%%%
\begin{equation}
R_{\rm stromgren} = \frac{3}{4\pi} \left( \frac{\dot N_X}{n^2 \beta } \right)^{1/3}
\end{equation}
%%%%%%%%%
where $\dot{N}_X$ is the source rate of ionizing photons, $n$ is the density of hydrogen, and $\beta $ is the coefficient for recombination \memoYF{cite something}:
...
\end{eqnarray}
%%%%%%%%%
The source rate is obtained by counting the number of photons whose energy exceeds the ionization energy of Hydrogen ($1{\rm~Ry} = 13.6$~eV where Ry is the Rydberg constant, or $\lambda = 91.2\rm~nm$), which is approximately given by dividing the X-ray accretion luminosity by the characteristic photon energy produced:
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\memoYF{how about the following}
...
Let us consider the ionization state around the planet. 
We suppose a state where the ionization and recombination rates are in equilibrium. 
The cross section of H atoms for X-ray, $\sigma _H [E_{\rm photon}] $ is
%%%%%%%%%
\begin{equation}
\sigma _H [E_{\rm photon}] = 6.3 \times 10^{-18} \;{\rm cm}^2 \cdot \left( \frac{E_{\rm photon}}{E_{\rm Rydberg}} \right)^{-3} 
\end{equation}
%%%%%%%%%
where $E_{\rm photon} = k_B T_{\rm acc} $ is energy per photon, 
while the recombination rate $\beta [T_e]$ is 
%%%%%%%%%
\begin{equation}
\beta [T_e] = 2.6 \times 10^{-13}\;{\rm cm^3/sec}\cdot\left(\frac{T_e}{10^{4}} \right)^{-3/4}
\end{equation}
%%%%%%%%%
where $T _e$ is electron temperature $\sim 10^4$ K \memoYF{cite something}. 

Denoting the ionization fraction by $\alpha $, the equilibrium state at a distance $r$ from the planet is represented by 
%%%%%%%%%
\begin{equation}
\frac{\dot N_X}{ 4 \pi r^2 } n ( 1-\alpha ) \sigma _H [E_{\rm photon}] = (n \alpha )^2 \beta [T_e] \label{eq:equilibrium}
\end{equation}
%%%%%%%%%
Here, we ignored the attenuation of X-ray because the optical depth $\tau \sim n \sigma r$ is much smaller than unity.  
The ionization fraction $\alpha $ that satisfies \ref{eq:equilibrium} is shown in Figure \ref{fig:ionizationfraction}.  
Although the photon number is large for larger planets, the strong dependence of cross section on the photon energy decreases the radius of ionized region. 
However, still, substantial amount of ionized plasmas are expected around the magnetic stand-off radius. 
\memoYF{Secondary ionization by energetic electrons needs to be incorporated.}


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[htbp]
   \plotoneh{ionizationfraction.pdf}
   \caption{Ionization fraction due to X-ray from the planetary accretion disk.}
...
\end{equation}
%%%
The radius obtained for Jupiter from this equality is about a half of the actual radius of magnetosphere \citep[][]{griesmeier2005}. 
...
%%%
\begin{eqnarray}
P_{\rm k,\,inp} &=& nv^3 r_s^2 %= n v^3 \left( \frac{B^2}{8\pi m_{\rm p}nv^2} \right)^{1/3} R_J^2  \\
\\
...
The continuum lines from the star in the cases of $\alpha =0.8$ and $2.0$ are shown as an extreme case. %; the 0.6 value is the prediction from the constant property wind model \citep{wright1975,panagia1975,olnon1975}. 
...
The potential factors that cause time variability are  listed below. 
...
%%%
\begin{equation}
P_{\rm inp,k}=nv^3 r_s^2 \propto \dot M_{\star} v^2 a^{-2} r_s^2.
\end{equation}
%%%
Combining the above equation with the scaling between the radius of the magnetic stand-off point, $r_s$, and the planetary magnetic field $B$ (equation (\ref{eq:stand-off-radius})), i.e.\
%%%
\begin{equation}
r_s \propto B^{1/3} a^{1/3} \dot M_{\star }^{-1/6} v^{-1/6}, 
\end{equation}
%%%
