...
\revise{Thus, }{In equation (\ref{eq:stand-off}), we ignore the ram pressure of the particles from the planets. %
}
%%%
\begin{equation}
...
\citet{spiegel+madhusudhan2012} \revise{}{considered Bondi–Hoyle accretion and} computed the accretion luminosity $L_{\rm acc}$ and the temperature $T_{\rm acc}$ as follows: 
%%%%%%%%%
\begin{eqnarray}
...
\revise{}{Note that in this case the accretion radius is}
%%%%%%%%%
\begin{eqnarray}
R_{\rm acc} = \frac{2GM_p}{v_{\rm rel}^2} \sim 
\end{eqnarray}
%%%%%%%%%
...
\revise{}{In reality, only the neutral portion of the stellar wind can accrete onto the planet without interacting with planetary magnetic field and thus Bondi-Hoyle accretion would be relevant. Therefore, what we could envision is...}
...
\revise{}{We note that in this range }
...
Assuming Jupiter-mass planets \revise{}{at 1 AU (5 AU)} which typically have the maximum cyclotron frequency at $10-100$~MHz depending on the age, \revise{a reasonable number of}{148 (44)} systems have potential to emit radio wave at the level above $\sim 10~\mu {\rm Jy}$  out to $\sim 100-300$ pc, while those with the highest mass-loss rate would suffer from the plasma cut-off of the stellar wind. 
...
Observations suggest that the occurrence rate of planets with masses $M_{\rm J} < M_p \sin i < 15 M_{\rm J}$ in orbits $1 {\rm AU} < a < 5 {\rm AU}$ around F, G, or K stars is $\sim $ 3.5\% \citep{cumming2008}. We could assume the rate does not change significantly in red giant stage, while admitting the likely orbital evolution at this stage. %
Then, given that 148 (44) systems with 1$M_{\rm J}$ planets at 1 AU (5 AU) have potential to emit $>10 \mu {\rm Jy}$, about 2-5 would actually be emitting the radio wave of our interest. %
For larger planets ($M_p>2M_{\rm J}$), the number of potentially detectable systems are less, due to the $B^{-1/3}$ dependence of the spectral flux: the number of systems where a 10 $M_{\rm J}$ planet at 1 AU (5 AU) emit $>10\mu {\rm Jy}$ is 21 (5). Since the occurrence rate of such larger planets in the same orbital region is $\sim 3.1\%$ \citep{cumming2008}, one could expect additional $\sim 1$ radio source detection from larger planets with sensitivity 10$\mu {\rm Jy}$. %
}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Effect of ram pressure due to planetary outflow}

At the red giant stage, the atmospheric temperature of planets increases to $> 1000 {\rm K}$. At this point, one might anticipate that planetary atmosphere starts to escape. 
Also, the UV/X-ray produced from accretion may input energy for atmospheric molecules to escape. In this section, we briefly estimate these effects on the circumplanetary configuration. 

The escape parameter of RGHJs is:
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{equation}
\lambda _c = \frac{G M_p m_{\rm p}}{k T_c r_c} = 214 \left( \frac{M_p}{M_{\rm J}} \right) \left( \frac{T_c}{1000 {\rm K}} \right)^{-1} \left( \frac{r_c}{R_{\rm J}} \right)^{-1}
\end{equation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
where $m_{\rm p}$ is mass of a hydrogen atom, $r_c$ is the radius of exobase. Therefore in most of the case the escape is safely assumed to be Jeans escape. 
Escape rate of Jeans escape is: 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{equation}
...
\Phi _{\rm Jeans} (r_c) &=& \frac{1}{\sigma } \sqrt{ \frac{2 G \bar \rho }{3} } \left( \lambda _c + 1 \right) \sqrt{ \lambda _c } e^{-\lambda _c} \\
...
\end{eqnarray}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
and the ram pressure at distance $r$ due to such an outflow is
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{eqnarray}
...
\end{eqnarray}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
The ram pressure of the stellar wind or planetary magnetic pressure (either hand of equation (\ref{eq:stand-off})) is $\sim 2.7 \times 10^{-12} ~{\rm Pa} $. 
This implies that the ram pressure of the planetary outflow would be negligible when $\lambda _c > 15$. [NEED TO CHECK]
