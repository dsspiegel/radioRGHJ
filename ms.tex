...
\end{eqnarray}
%%%%%%%%%
Therefore, the accretion disks around planets therefore emit UV/X-ray radiation 
...
Denoting the ionization fraction by $\alpha $, the equilibrium state at a distance $r$ from the planet may be approximately represented by 
%%%%%%%%%
\begin{equation}
...
In equation (\ref{eq:equilibrium}), we ignored the attenuation of X-ray because the optical depth $\tau \sim n \sigma _H r$ is much smaller than unity.  
...
\end{eqnarray}
%%%%%%%%%
For the accretion disks of large planets, however, the characteristic photon energy is so high (equation (\ref{eq:Tacc})) that the released energetic electrons may also ionize other atoms in the vicinity; the cross section of hydrogen for electrons is $\sim 4\times 10^{-17} {\rm cm}^2$  \citep{fite1958} and thus about 60\% of the released electrons will be used to ionize other atoms within a distance of $2R_J$. 
In theory, photons with energy $E{\rm photon}$ have potential to ionize $E_{\rm photon}/E_{\rm Rydberg}$ times. 
To take this into account, we also consider replacing the source term $\dot N_X$ in equation (\ref{eq:equilibrium}) by 
%%%%%%%%%
\begin{equation}
\dot{N}_X \rightarrow \dot{N}_X \left[ 1 + f \left( \frac{E_{\rm photon}}{E_{\rm Rydberg}} - 1 \right) \right] 
\end{equation}
%%%%%%%%%
where the $f$ is a fudge factor to represent the efficiency of this reaction. 

The ionization fraction $\alpha $ as a solution of equation (\ref{eq:equilibrium}) is shown in Figure \ref{fig:ionizationfraction}. 
The solid lines show the ionization fraction of $f=0$ (no additional ionization by electrons) and dashed lines show that of $f=0.5$. In the figure, the vertical lines show the magnetic stand-off points obtained by equation (\ref{eq:stand-off-radius}) below. \memoYF{need to replace the order?} 
...
The Stromgren radius are indicated by vertical arrows in Figure \ref{fig:ionizationfraction}. 


%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[htbp]
   \plotoneh{ionizationfraction.pdf}
   \caption{Profile of ionization fraction due to X-ray from the planetary accretion disk, measured from the surface of the planet. Solid lines show the solutions without the correction for the secondary ionization by ionized electrons, while the dashed lines show the solutions which take the correction into account with the fudge factor $f=0.5$. The vertical arrows show the Stromgren radius estimated simply  by equation (\ref{eq:stromgren}). The dotted vertical lines indicate the location of the magnetic stand-off points obtained by equation (\ref{eq:stand-off-radius}) below. }
