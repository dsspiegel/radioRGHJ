...
%%% this part is to be removed eventually %%%
...
%
David S. Spiegel\altaffilmark{3,4,5}
%
Tony Mroczkowski\altaffilmark{6,7}
%
Jason Nordhaus\altaffilmark{8,9}\\
%
Neil T. Zimmerman\altaffilmark{10}
%
Aaron Parsons\altaffilmark{11}
%
Mehrdad Mirbabayi\altaffilmark{5}
%
Nikku Madhusudhan\altaffilmark{12}
}
...
\affil{$^2$NASA Goddard Institute for Space Studies, 
  New York, NY 10025}
    
\affil{$^3$Analytics \& Algorithms, Stitch Fix,
  San Francisco, CA  94103}

\affil{$^4$Research \& Development, Sum Labs,
  New York, NY 10001}

\affil{$^5$Astrophysics Department, Institute for Advanced Study,
  Princeton, NJ 08540}

\affil{$^6$National Research Council Fellow}

\affil{$^7$Naval Research Laboratory, 4555 Overlook Ave SW, Washington, DC 20375}

\affil{$^8$Department of Science and Mathematics, National Technical Institute for the Deaf, Rochester Institute of Technology, Rochester, NY 14623}

\affil{$^9$Center for Computational Relativity and Gravitation, Rochester Institute of Technology, Rochester, NY 14623}

\affil{$^10$Space Telescope Science Institute, 3700 San Martin Drive, Baltimore, MD 21218}

\affil{$^{11}$Astronomy Department, University of California Berkeley}

\affil{$^{12}$Astronomy Department, University of Cambridge, UK}
...
\revise{\citet{jardine2008} modeled reconnection between the magnetic field of a close-in planet and that of the host star to obtain estimates that are not based on the ``radiometric Bode's law''.}
Although observational searches for these radio signatures are underway, no clear detection has been claimed \revise{\citep{bastian2000,george2007,smith2009,stroe2012,hallinan2013,murphy2015}}, while there are some promising initial results \citep{lecavelier_et_al2013,sirothia2014}.
...
\revise{In addition, they considered a scenario in which post-bow shock heating could produce ionized hydrogen atoms. This indicates a configuration of radio-wave generation very different from that of Solar System, and the lower-limit estimate, at least, is well below the detection limit.}

In this paper, we \revise{consider another plausible scenario where} the accretion of the massive stellar wind onto the planet would emit UV and X-ray photons that ionize the stellar wind in the vicinity of the planet.
...
\revise{In addition, we introduce two major advances beyond the work of \citet{ignace2010} that are related to the observability of radio emission from RGHJs.}  % I don't like referring to our work as "updates" from Ignace
\revise{First, we estimate} the plasma frequency cut-off of the stellar wind, which turns out to be one of the major obstacles to detecting radio emission from RGHJs.
\revise{Second, we discuss the planetary parameter space to search for radio emission, employing scaling laws for the planetary magnetic field, noting that the survey of exoplanets around highly evolved stars is not complete.}

In Section \ref{s:assumptions}, we present the framework to obtain the frequency and the flux of planetary auroral radio emission, and describe our models for the stellar wind and planetary magnetosphere.
...
In reality, the total power of Jovian auroral emission varies greatly over time: $1.3\times 10^{10}$~W for the average, \revise{$8.2\times 10^{10}$}~W for the average of highly active time, and $4.5 \times 10^{11}$~W for the peak activity \citep{zarka_et_al2004}. 
...
\revise{We note that the outflow ram pressures from heated planets are negligible compared to these pressures, as discussed in Appendix \ref{ap:outflow}.
}
Thus, 
%%%
\begin{equation}
...
While $v_{\rm sw}$ is $\sim$30~km/s, the infall velocity from the planetary gravity is $\sim$10-25~km/s depending on the planetary mass (ranging from $1 M_{\rm J}-10 M_{\rm J}$), and the orbital velocity is $10-30~{\rm km/s}$ depending on the orbital distance (ranging from $1-5~{\rm AU}$). 
...
As discussed in \citet{ignace2010}, as the stars evolve, the ionization fraction of the stellar wind diminishes to of order $\sim$10$^{-3}$ \citep{drake1987}.
Since only charged particles interact with planetary magnetic field, this suggests inefficient interaction with planetary magnetosphere and hence low input energy for radio emission.
However, at this stage, the velocity of the stellar wind becomes slower than the escape velocity of the planetary companion and hence the stellar wind particles are expected to accrete onto the planets. 
\citet{spiegel+madhusudhan2012} \revise{considered Bondi--Hoyle accretion where the accretion radius is}
%%%%%%%%%
\begin{eqnarray}
R_{\rm acc} &=& \frac{2GM_p}{v_{\rm rel}^2} \\
&\sim & 4.0~R_{\rm J} \left( \frac{M_p}{M_{\rm J}} \right) \left( \frac{v_{\rm rel}}{30 ~{\rm km~s^{-1}}} \right)^{-2} \, .
\end{eqnarray}
%%%%%%%%%
\revise{The accretion luminosity $L_{\rm acc}$ and temperature $T_{\rm acc}$ are}
%%%%%%%%%
\begin{eqnarray}
L_{\rm acc} \sim &&  10^{25} \; {\rm erg\;s}^{-1} \left( \frac{\dot M_*}{10^{-8} M_{\odot }/{\rm yr}} \right)  \notag \\
&& \times \left( \frac{M_p}{M_{\rm J}} \right)^3 \left( \frac{M_*}{M_{\odot }} \right) \, . \label{eq:Lacc}\\
%
T_{\rm acc} \sim && 2 \times 10^5 ~\mbox{K} \left( \frac{M_p}{M_{\rm J}} \right) \, . \label{eq:Tacc}
\end{eqnarray}
%%%%%%%%%
The accretion onto planets, therefore, leads to emission of UV/X-ray radiation whose characteristic energy $\sim$$k_B T_{\rm acc}$ exceeds the ionization energy of hydrogen, $E_{\rm Rydberg} = 13.6$~eV, or $\lambda = 91.2\rm~nm$.
...
\end{equation}
%%%%%%%%%
where $E_{\rm photon}$ is the energy per photon, and $\beta [T_e]$ is the ``class B'' recombination coefficient which is $\beta \sim 2.6 \times 10^{-13}\;{\rm cm^3/sec} $ \citep{pequignot1991} at the electron temperature of the stellar wind of evolved stars, $T_e \sim 10^4$~K \citep{suzuki2007}. 


The source rate is obtained by counting the number of photons with energy exceeding the ionization energy of hydrogen ($E_{\rm Rydberg} = 13.6$~eV, or $\lambda = 91.2\rm~nm$), which is approximately given by dividing the X-ray accretion luminosity by the characteristic photon energy produced:
%%%%%%%%%
\begin{equation}
...
The cross section\footnote{This is close to the geometric cross section of the Bohr radius.} of hydrogen for electrons is $\sim$4$\times$10$^{-17}$~cm$^2$ \citep{fite1958}, which implies a mean free path for ionized electrons of $\sim$2$R_J$ in the surrounding medium.
...
When, $\tau \sim n \sigma_H r$ is much smaller than unity and thus $e^{-\tau}$ term can be ignored (that is the case for $M_p = 10 M_{\rm J}$),  the solution is simply
%%%%%%%%%
\begin{eqnarray}
x[r] &=& \frac{-1 + \sqrt{1+4C[r]}}{2C[r]} \\
C[r] &\equiv &   \frac{4 \pi n \beta [T_e] r^2}{\dot{N} \sigma_H[E_{\rm photon}]}  \end{eqnarray}
%%%%%%%%%
The solid lines show the ionization fraction corresponding to no additional ionization by electrons (i.e., $\xi=0$, or $N_i = 1$) and \revise{the dashed line shows the solution with $N_i=6$ for a $10 M_{\rm J}$ planet}.
...
\revise{In reality, the temperature and luminosity based on the Bondi--Hoyle accretion (equations (\ref{eq:Lacc} and (\ref{eq:Tacc})) describes the situation only approximately.
Precisely, only the neutral portion of the stellar wind can accrete onto the planet without interacting with planetary magnetic field, and the ionized portion would lose some energy at the bow shock before it accretes.
On the other hand, the ionized plasmas interacting with magnetic field could have a cross section larger than the Bondi--Hoyle accretion radius.
The detailed electromagnetic structure around RGHJs therefore requires elaborate numerical simulations, which is beyond the scope of this paper.
Noting these uncertainties, in this paper we simply aim to give an order-of-magnitude estimate.}
...
In this regime, there are a number of current and near-future radio wave detectors including GMRT, LOFAR, HERA, SKA, and potential upgrades to the VLA (see Section \ref{ss:detectability}). 
...
\revise{We also note that the orbital velocity of the canonical hot Jupiters has been ignored in equation (\ref{eq:P_HJ}); the orbital velocity at 0.05~AU around a Solar-mass star is $\sim$130~km~sec$^{-1}$, which contributes to only $\sim$5\% increase of the velocity.}
...
Compared with the equivalent Jupiter-like planets around main sequence stars, the massive stellar wind of late red giants can increase the radio emission from planetary companions by more than two orders of magnitude, which allow us to explore 10 times more distant system, i.e., 1000 times more volume.
This at least partially compensates for the small population of evolved stars.

Equation (\ref{eq:F_nu_RGHJ}) gives a spectral flux density that 1---2 orders of magnitude smaller than the prediction of equation (5) in \citet{ignace2010} if we assume the same set of fiducial values and full ionization.
This is primarly because their formulation didn't incorporate the effect of compressed planetary magnetosphere due to massive stellar wind, while the scaling law for the planetary magnetic field strength is also different.
...
The doubly hatched regions in Figure \ref{fig:radio} indicate the parameter spaces where 
%the criteria $\nu_{\rm cyc,\,max} > \nu_{\rm plasma}^{\rm los}$ is not satisfied and 
\revise{$\nu_{\rm cyc,\,max} < \nu_{\rm plasma}^{\rm los}$ and there is no hope to observe it from below the ionosphere.}
%thus the radio emission from the planet cannot be observed from the Earth. 
...
%   \plotonesc{cartoon_10Mj_100pc_smooth.pdf}
   \caption{A cartoon of the radio emission spectra of a RGHJ with 10$M_{\rm J}$ and the host red giants with 100 $R_{\odot }$.
     ...
     \revise{The shaded region represents the portion of the spectrum below the plasma frequency of the stellar wind.}}
  \label{fig:cartoon}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
\revise{We note that their data do not unambiguously distinguish RG from early AGB stars and so about 40\% of the list probably consists of early AGB stars. }\memoYF{grammatical question: doesn't ``consists of'' sound like ``A is made of B''?}
...
\end{equation}
%%%
The age of the system is assumed to be roughly the same as the lifetime of the main-sequence stage, which depends on the stellar mass.
We simply assume:
%%%
\begin{equation}
...
\end{equation}
%%%
In addition, the distance to the system is obtained based on the parallax data from Hipparcos datasets\footnote{\url{http://www.rssd.esa.int/index.php?project=HIPPARCOS}}.
...
Assuming Jupiter-mass planets \revise{at 1 AU (5 AU)} which typically have the maximum cyclotron frequency at $10-100$~MHz depending on the age, \revise{148 (12)} systems have potential to emit radio waves at the level above $\sim$10$~\mu$Jy out to $\sim$100-300~pc, while those with the highest mass-loss rates would suffer from the plasma cut-off of the stellar wind. 
For more massive planets with the maximum cyclotron frequency at around $100-1000$~MHz, the radio flux density is lowered due to its dependence on $B^{-1/3}$ (equation (\ref{eq:F_nu})), but more likely to get to the Earth.
$10~M_{\rm J}$ planets at $\sim$1~AU may be detectable out to $\sim$200~pc.

\revise{A more realistic expected number of detection may be evaluated by combining these results with the probability for a star to have a planet with certain mass and orbit,  %
Observations of main-sequence stars imply that the number of Jovian planets is about a few to ten percent of the number of stars \citep[e.g.][]{cumming2008,cassan2012}. %
Assuming the same population for RG systems, the expected number of actual detection from RGHJs would be of order unity. 
...
%However, as we see in the next subsection, the . and the expected value for massive planets ($M_p>5M_{\rm J}$) which supposedly have relatively high cyclotron frequency is about 1-2. 
...
% VLA
For the low frequency receivers (28-80~MHz; 4-Band) in the Very Large Array (VLA) have $\Trx \sim 260~\rm K$, similar to the receiver noise temperatures for the Long Wavelength Array (LWA) \citep{Hicks2012}.
As noted in \citep{Hicks2012}, this noise level is subdominant to \Tsky\ by at least -6~dB.
...
The probability of the occurrence of the Jovian radio burst is correlated with the spin rotation phase \citep[e.g.][]{dessler1983} due to the misalignment between the magnetic axis and the spin axis.
...
\revise{%
Once planetary radio emission is detected, it provides a unique approach for studying the planet's property.
As the auroral radio flux is significantly modulated in association with several physical processes described in \S\ref{ss:timevariability}, the time variability is useful to infer the planetary spin rotation period or the presence of satellites.
If spectra are obtained, the upper cut-off frequency can tell us the magnetic field strength at the planetary surface, which, when combined with constraints on the planetary mass via e.g.\ radial-velocity observations, tests the proposed scaling law of planetary dynamo. 
Furthermore, the lower cut-off frequency of the RGHJ emission, which presumably corresponds to in situ plasma frequency, probes the stellar wind properties at the planetary orbit.
In addition, the planetary radio power in such an extreme environment as a RG's massive stellar wind will be valuable data points to test the empirical ``radiometric Bode's law''.
We also note that the radio emission of RGHJs may be used as a tool for discovering/confirming new planets, given that traditional exoplanet-discovery methods (specifically, transit and radial-velocity observations) are not very sensitive to planets around RGB or AGB stars.
}
...
YF is supported from the Grant-in-Aid No. 25887024 by the Japan Society for the Promotion of Science and from an appointment to the NASA Postdoctoral Program at NASA Ames Research Center, administered by Oak Ridge Affiliated Universities.
DSS gratefully acknowledges support from a fellowship from the AMIAS group.
JN acknowledges support from NASA grant HST AR-12146.04-A and NSF grant AST-1102738.
...
The portion of this research for which TM is responsible was performed under a National Research Council Research Associateship Award at the Naval Research Laboratory (NRL).
...
%One might anticipate that these factors increase the escape of planetary atmospheres, which could affect the pressure balance represented by equation (\ref{eq:stand-off}). 
...
\end{equation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
where $T_c$ and $r_c$ are the radius and temperature at exobase.
In most of the cases we consider, therefore, the escape is safely assumed to be Jeans escape. 
The rate of Jeans escape is 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\begin{equation}
%\dot M = 4 \pi \rho_0 a r^2 \left( \frac{v_{\rm esc}(r_0)}{2a} \right)^2 \exp \left( - \frac{v_{\rm esc}^2 (r_0) }{2 a^2} + \frac{3}{2} \right)
%\end{equation}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{eqnarray}
%\Phi _{\rm Jeans} &=& 4 \pi r_c^2 \cdot \frac{n_c}{2 \sqrt{\pi}} B \sqrt{\frac{2 k_B T_c }{m}} ( 1 + \lambda _c ) e^{-\lambda _c} \\
\Phi _{\rm Jeans} (r_c) &=& \frac{1}{\sigma } \sqrt{ \frac{G M_p}{2 R_p^3} } \left( \lambda _c + 1 \right) \sqrt{ \lambda _c } e^{-\lambda _c} \\
%&=& 1/(pi*(0.529e-10)**2)*sqrt(2*6.674e-11*1330./3.0) * (20+1.0)*sqrt(20.)*exp(-20.0) 
%&=& 1/(pi*(0.529)**2)*sqrt(2*66.74*1330./3.0) * (20+1.0)*sqrt(20.)*exp(-20.0) * 1e14
\end{eqnarray}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
% 1.674e-27 * 5.3e18 / 100.0 * 59.5e3 
%&=& 5.27 \times 10^{-6} ~{\rm Pa} \; \left( \frac{r}{10~R_p} \right)^{-2} \left( \frac{M_p}{M_{\rm J}} \right)^{1/2}
\end{eqnarray}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
which indicates $3.6\times 10^{-7} {\rm~dyne~cm^{-2}}$ even when $\lambda _c = 1$. % DSS: barye is a unit that no one uses
On the other hand, the ram pressure of the stellar wind or planetary magnetic pressure (either hand of equation (\ref{eq:stand-off})) is $\sim$2.7$\times$10$^{-5} {\rm~dyne~cm^{-2}}$. 
...
This implies that in this range the above estimate may actually be reliable.
The transfer momentum is
\begin{equation}
...
When the particle spirals into the planet, the magnetic pressures increases as 
%%%
\begin{equation}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Expected Number of Detection Taking Account of Planet Occurrence Rate}
\label{s:expectedvalue}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

Let us also consider the expected number of radio detection from RGHJs taking account of the occurrence rate of Jovian planets. 
For this purpose, we consider an empirical power-law planet occurrence rate described as
%%%
\begin{eqnarray}
f(M_p,P) &\equiv & \frac{d^2N}{d\log M_pd\log P } \\
&=& \mathcal{C} \left( \frac{M_p}{M_{\rm J}} \right)^{\alpha } \left( \frac{P}{1~{\rm days}} \right)^{\beta } \, ,
\end{eqnarray}
%%%
where \citet{cumming2008} found $\alpha = -0.31\pm 0.2$, $\beta =0.26 \pm 0.1$, $\mathcal{C} =1.04\times 10^{-3}$ as best-fit values. %
Using this equation, the probability of a given evolved star in Figure \ref{fig:observability} to have a RGHJ with mass larger than threshold $M_{p0}$ whose radio emission may be detectable is
%%%
\begin{equation}
\mathcal{P }(>M_{p0}) \sim \int_{M_{p0}} d\log M_p\,\int_{P_{\rm min}(M_p)}^{P_{\rm max}(M_p)} d\log P ~f(M_p, P) \, , \label{eq:detect_prob}
\end{equation}
%%%
where $P_{\rm min}(M_p)$ is the minimum orbital period to have cyclotron frequency larger than the ambient plasma frequency, while $P_{\rm max}(M_p)$ is the maximum orbital period given planetary mass to emit detectable radio flux.
For the former, combining equations (\ref{eq:fcyc}), (\ref{eq:scalingB}), and (\ref{eq:fplasma_RG}) leads to
%%%
\begin{eqnarray}
a_{\rm min} (M_p) &=& 2.1~{\rm AU} \left( \frac{M_p}{M_{\rm J}} \right)^{-1.04} \left( \frac{t}{4.5~{\rm Gyr}} \right)^{0.43}  \\
&&\times \left( \frac{\dot M_{\star }}{10^{-8}M_{\odot}/{\rm yr}} \right)^{1/2} \, ,
\end{eqnarray}
%%%
while for the latter, equations (\ref{eq:scalingB}) and (\ref {eq:F_nu_RGHJ}) indicate
%%%
\begin{eqnarray}
a_{\rm max} (M_p) &=& 3.8~{\rm AU} \left( \frac{F_{\rm min}}{10~\mu {\rm Jy}} \right)^
{-3/4} \left( \frac{d}{100~{\rm pc}} \right)^{-3/2} \\
&& \times \left( \frac{M_p}{M_{\rm J}} \right)^{-1.04/4}  \left( \frac{t}{4.5~{\rm Gyr}} \right)^{0.43/4} \\
&& \times \left( \frac{\dot M_{\star }}{10^{-8}M_{\odot }/{\rm yr}} \right)^{1/2} \left( \frac{v_{\star }}{10^{-1} v_{\odot }} \right)^{5/4} \, .
\end{eqnarray}
%%%
They are translated into the orbital period via Kepler's law, $P_{\{{\rm min,max}\}} = 2\pi \sqrt{a^3_{\{{\rm min,max}\}} / GM_{\star } }$. 
...
Using equation (\ref{eq:detect_prob}), the expected number for planets more massive than $M_{p0}$ with detectable radio emission is $\sum _i \mathcal{P} _i(>M_{p0})$, where $\mathcal{P}_i(>M_{p0})$ is the probability for the $i$-th target. 

Figure \ref{fig:expected_values} show the expected number of detection as a function of threshold planetary mass $M_{p0}$.
This figure indicates that, if all of the target in Figure \ref{fig:observability} are red giant stars, the total number of detections would be about 4 (7), provided that the sensitivity of the radio instruments $F_{\rm min}$ is $10~\mu {\rm Jy}$ ($5~\mu {\rm Jy}$). 
...
Observations suggest that the occurrence rate of planets with masses $M_{\rm J} < M_p \sin i < 2 M_{\rm J}$ in orbits $1 {\rm AU} < a < 5 {\rm AU}$ around F, G, or K stars is $\sim$3.9\% \citep{cumming2008}. %
%(Here we assume the rate does not change significantly in red giant stage, while admitting the likely orbital evolution at this stage. ) %
Given that 148 (12) systems with 1$M_{\rm J}$ planets at 1 AU (5 AU) have potential to emit $>10~\mu {\rm Jy}$, about 2-6 would actually be emitting the radio wave of our interest at the level of $>10~\mu {\rm Jy}$. %
For larger planets ($M_p>2M_{\rm J}$), the number of potentially detectable systems are less, due to both the $B^{-1/3}$ dependence of the spectral flux: the number of systems where a 10 $M_{\rm J}$ planet at 1 AU (5 AU) emit $>10~\mu {\rm Jy}$ is 98 (5): Since the occurrence rate of such larger planets in the same orbital region is $\sim$3.1\% \citep{cumming2008}, one could expect additional $\sim$1 radio source detection from larger planets with sensitivity $10~\mu {\rm Jy}$.
...
& \citet{reiners2010} & $P_{\rm radio} \propto P_{\rm sw}$ & $\mathcal{M} \propto M_p^{1/6} L^{1/3} R_p^{25/6}$ \\ 
\#8  & this work & & \\ \hline
\end{tabular}
\end{center}
\label{tab:comp}
...
If we adopt the assumptions that $R_p \sim R_J\;\;(const.)$, $\rho _c\sim R_p\sim const.$, and $\sigma = const.$, then the scaling laws of the auroral radio power are reduced to:
%%%%%%%%%
\begin{eqnarray}
...
%
\mbox{[\#8]} \; P_{\rm rad} &\sim & 2.1 \times 10^{11} \,{\rm W}\, \left( \frac{M_p}{M_{\rm J}} \right)^{0.70}  \left( \frac{t}{4.5 \rm~Gyr} \right)^{-0.29}  \left( \frac{a}{a_{\rm J}} \right)^{-4/3} \left( \frac{\dot M_\star}{\dot M_{\odot}} \right)^{2/3} \left( \frac{v}{v_{\odot}} \right)^{5/3} \, \\
\end{eqnarray}
%%%%%%%%%
