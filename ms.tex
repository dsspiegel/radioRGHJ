...
%   \plotonesc{cartoon_10Mj_100pc_smooth.pdf}
   \caption{A cartoon of radio emission spectra of a RGHJ with 10$M_{\rm J}$ and the host red giants with 100 $R_{\odot }$.
     ...
     \revise{The shadowed region represents the frequency below the plasma frequency of the stellar wind.}}
  \label{fig:cartoon}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% 
...
\revise{%
Once planetary radio emission is detected, it provides a unique approach for learning the planetary property.
As the auroral radio flux is significantly modulated in association with several physical processes described in \S\ref{ss:timevariability}, the time variability is useful to imply the planetary spin rotation period or the presence of satellites.
If spectra are obtained, the upper cut-off frequency can tell us the magnetic field strength of the planetary surfaces, which, when combined with constrains on the planetary mass via e.g. radial-velocity observations, tests  the proposed scaling law of planetary dynamo. 
...
We also note that the radio emission of RGHJs may also be used as a tool for discovering/confirming new planets, given that traditional exoplanet-discovery methods (specifically, transit and radial-velocity observations) are not very sensitive to planets around RGB or AGB stars.
}
