...
\memoTM{Since they are not that bright, maybe ``The Radio Emission from Red-Giant Hot Jupiters'' 
or ``A Novel Method for Detecting Red-Giant Hot Jupiters'' would be a better title?}}
...
\affil{$^8$Center for Computational Relativity and Gravitation, Department of Mathematics, Rochester Institute of Technology, Rochester, NY 14623}
...
We estimate the spectral auroral radio intensity of RGHJs employing the ``radiometric Bode's law'' and a proposed scaling for planetary magnetic fields.
We predict that RGHJs might be as bright as canonical hot Jupiters, and thus the low frequency radio emission from a RGHJ may be detectable at distances less than a few hundred parsecs with current/future instruments such as LOFAR and SKA.
...
The time-dependent luminosity is taken from \citet{burrows_et_al2001} \citep[see also][]{marley2007}. 
...
%%%%%%%%%% 
\begin{eqnarray}
n &=& 1.80 \times 10^6 \; {\rm cm^{-3}} \times \left( \frac{a}{5 \; {\rm AU}} \right)^{-2} \notag \\
...
\end{eqnarray}
%%%%%%%%%
The source rate is obtained by counting the number of photons whose energy is greater than 13.6~eV ($\lambda < 91~\rm nm$), the energy needed to ionize hydrogen atoms.
As a result, 
%%%%%%%%%
\begin{eqnarray}
S_* \sim  
...
%%%%%%%%%% 
\begin{equation}
\nu_{\rm cyc} = \frac{eB}{2\pi m_e} \approx 27.9 {\rm~MHz} \left( \frac{B}{10 \rm~G} \right) \label{eq:fcyc}
\end{equation}
%%%%%%%%%%
...
%%%
\begin{eqnarray}
\label{eq:fplasmalos} \nu_{\rm plasma}^{\rm los} & \sim & 8.98 {\rm~kHz} \times \left( \frac{\dot M_\star}{4 \pi a^2 m_p v} \times 1\rm~cm^3 \right)^{1/2} \\
...
Reasonably, the resultant values are in good agreement with \citet{reiners2010}, who adopted the same scaling law for planetary magnetic field; we show this figure for completeness. 
...
Depending on the modeling of magnetic field strength, there is also speculation that the tidally-locked planets may have weaker magnetic fields due to slow rotation \citep[e.g.][]{griesmeier2004}; in that case the radio emission would be even weaker. 
...
In reality, the peak of the auroral radio emission does not always occur at the maximum frequency. Instead, it usually exits at lower frequencies. Here, we also consider a more conservative criteria, $0.1 \nu_{\rm cyc} > \nu_{\rm plasma}^{\rm los}$ as a measure for the observability of the bulk of radio emission. In Figure \ref{fig:radio}, this conservative region is shown as hatched regions with vertical lines. 
It is now shown that the plasma cut-off due to massive stellar wind is a major obstacle in detecting planetary companions around evolved stars. For a red-giant system, the most promising target is a massive companion with $M_p \gtrsim 5M_J$. On the other hand, for a AGB-star system, it is very difficult to detect a signal from any planetary-mass companions within 100 AU. 
...
Finally, \S\ref{ss:timevariability} discusses the expected time variability of the radio emission as a means to discern the signals from the background noise. 
...
%
\citet{OGorman2013} reported $\alpha $ Boo ($R_\star = 25.4 R_{\odot }$, $T=4286$ K, $d=11.26$ pc) at 1~GHz is about 70 $\mu$Jy, and $\alpha $ Tau ($R_\star = 44.2  R_{\odot }$, $T=3910$ K, $d=20.0$ pc) at 3~GHz is about 40 $\mu$Jy. 
...
In order to examine the potential to detect radio emission of RGHJs with current and near-future instruments, we estimate the possible radio emission from known red giants assuming that they have planetary companions. 
...
\memoTM{For this purpose, we obtained the masses, radii, and effective temperatures for a catalog of M-type red giants \citet{dumm1998} (in their Table 4).}
...
\end{equation}
%%%
where $R_{\star }$, $L_{\star } (=4\pi R_{\star }^2 T_{\star }^4)$, and $M_{\star}$ are the radius, luminosity, and mass of the star. 
...
One of the key features that makes planetary radio emission distinct is the almost complete circular polarization \citep[e.g.][]{dulk1994}.
