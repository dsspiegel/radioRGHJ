...
We estimate the spectral auroral radio intensity of RGHJs employing the ``radiometric Bode's law'' and a proposed scaling for planetary magnetic fields. \memoDS{I recommend nixing ``radiometric Bode's law'' from the abstract, but wanted to get others' opinions before doing so.}
We predict that RGHJs might be intrinsically brighter than canonical hot Jupiters, and thus the low frequency radio emission from a RGHJ may be detectable at distances up to a few hundred parsecs with current/future instruments such as LOFAR and the Square Kilometer Array.
...
%%%%%%%%%
\begin{equation}
\frac{\dot N_X}{ 4 \pi r^2 } n ( 1-x ) \sigma _H [E_{\rm photon}] = (n x )^2 \beta [T_e] \label{eq:equilibrium}
\end{equation}
%%%%%%%%%
...
Photons with energy $E_{\rm photon}$ have potential to ionize $E_{\rm photon}/E_{\rm Rydberg}$ times. 
To take this into account, we also consider replacing the source term $\dot N_X$ in equation (\ref{eq:equilibrium}) by 
%%%%%%%%%
\begin{equation}
\dot{N}_X \rightarrow \dot{N}_X \left[ 1 + \xi \left( \frac{E_{\rm photon}}{E_{\rm Rydberg}} - 1 \right) \right] 
\end{equation}
%%%%%%%%%
where the $\xi$ is a fudge factor to represent the efficiency of this reaction. \memoDS{Does the above argument imply that $\xi \sim 0.6$?  Should we think of it as an $N$-Rydberg photon, if absorbed, leads to 1 $N-1$-Rydberg electron, which produces 0.6 $N-2$-Rydberg electrons, $0.6^2$ $N-3$-Rydberg electrons, ... and $0.6^{N-1}$ 0-Rydberg electrons, so in the end we have $1 + 0.6 + 0.6^2 + ... + 0.6^{N-1} \sim 2.5$ electrons?}
% N - power - 1 = energy of electrons
% N - (N-1) - 1 = 0-Rydberg electron

The ionization fraction $x$ as a solution of equation (\ref{eq:equilibrium}) is shown in Figure \ref{fig:ionizationfraction}. \memoDS{We should write the expression ... it's just the quadratic formula, but it would be useful to write.}
The solid lines show the ionization fraction of $\xi=0$ (no additional ionization by electrons) and dashed lines show that of $\xi=0.5$. In the figure, the vertical lines show the magnetic stand-off points obtained by equation (\ref{eq:stand-off-radius}) below. \memoYF{need to replace the order?} 
...
\begin{figure}[htbp]
   ...
   \caption{Profile of ionization fraction due to X-ray from the planetary accretion disk, measured from the surface of the planet. Solid lines show the solutions without the correction for the secondary ionization by ionized electrons, while the dashed lines show the solutions which take the correction into account with the fudge factor $\xi=0.5$. The vertical arrows show the Stromgren radius estimated simply  by equation (\ref{eq:stromgren}). The dotted vertical lines indicate the location of the magnetic stand-off points obtained by equation (\ref{eq:stand-off-radius}) below. }
