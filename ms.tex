...
%%% \sout will be removed before submission %%%
\def\revise#1#2{\color{blue}\sout{#1} {\bf #2} \color{black}}
...
In this paper, we \revise{argue that}{consider another plausible scenario where} the accretion of the massive stellar wind onto the planet would emit UV and X-ray photons that ionize the stellar wind in the vicinity of the planet.
...
Thus, by extrapolating the ``radiometric Bode's law,'' we provide more optimistic estimates \revise{for highly evolved stars}{} compared to the previous result. 
\revise{}{In addition, two major updates from \citet{ignace2010} are the following.}
\revise{We also consider}{First, we estimate} the plasma frequency cut-off of the stellar wind, which turns out to be one of the major obstacles to detect radio emission from RGHJs. 
\revise{Given that the survey of exoplanets around highly evolved stars is not complete, we do not constrain ourselves to the systems known to have sub-stellar companions.
Instead, we investigate the parameter space where we can search for radio emission and examine the possibility to detect them with current and future instruments. }{Second, we discuss the planetary parameter space to  search for radio emission, employing the scaling law of planetary magnetic field, noting that the survey of exoplanets around highly evolved stars is not complete.}
...
\revise{Section \ref{s:discussion} is devoted to additional discussions, one for the usefulness of the radio emission to characterize the system and the other for the effects of canceling magnetic field which results from the plasmas spiraling in the planetary magnetosphere.}  
...
In reality, the total power of Jovian auroral emission varies greatly over time: $1.3\times 10^{10}$ W for the average, \revise{$3.2\times 10^{10}$}{$8.2\times 10^{10}$} W for the average of highly active time, and $4.5 \times 10^{11}$ W for the peak activity \citep{zarka_et_al2004}. 
...
As discussed in \citet{ignace2010}, as the stars evolve, the ionization fraction of the stellar wind is lowered down to order of $\sim 10^{-3}$ \citep{drake1987}. Since only charged particles interact with planetary magnetic field, this suggests inefficient interaction with planetary magnetosphere and hence low input energy for radio emission. 
...
\revise{This implies that the stellar wind particles represented by equation (\ref{eq:n}) are nearly fully ionized plasmas like Solar wind, and that the stellar wind can interact with the planetary magnetic fields in a rather similar way to the case of Solar System planets.}{}

\revise{}{In reality, only the neutral portion of the stellar wind can accrete onto the planet without interacting with planetary magnetic field and thus Bondi-Hoyle accretion would be relevant. 
The ionized portion could lose some of their energy at the bow shock before they accrete, and energy released there is less than the estimate by equations (\ref{eq:Lacc}) and (\ref{eq:Tacc}). 
%Now the stellar wind is so dense that the mean free path is comparable to the Jovian radius, the neutral and ionized particles may also drag each other. 
%While interacting with magnetic field, the ionized portion could lose their energy 
%Therefore, some part of the incoming wind particles are what we could envision is the 
%Considering other uncertainties of radio wave generation, we 
The detailed electromagnetic structure around RGHJs therefore requires  elaborate numerical simulations, which is beyond the scope of this paper. Noting these uncertainties, in this paper we simply aim to give an order-of-magnitude estimate. }
...
\revise{Therefore, for Jupiter-mass planets, only distant ones $\gtrsim 5 $  may emit the radio waves from the system. }{}
...
\revise{}{$\nu_{\rm cyc,\,max} < \nu_{\rm plasma}^{\rm los}$ and there is no hope to observe from Earth.}
...
Assuming Jupiter-mass planets \revise{}{at 1 AU (5 AU)} which typically have the maximum cyclotron frequency at $10-100$~MHz depending on the age, \revise{a reasonable number of}{148 (12)} systems have potential to emit radio wave at the level above $\sim 10~\mu {\rm Jy}$  out to $\sim 100-300$ pc, while those with the highest mass-loss rate would suffer from the plasma cut-off of the stellar wind. 
...
%\revise{}{The expected values for these targets are }
...
\revise{%
Finally, we note that the radio emission considered in this paper can either be an avenue to learn more about known RGHJs or a tool for discovering new planets, given that traditional exoplanet-discovery methods (specifically, transit and radial-velocity observations) are not very sensitive to planets around RGB or AGB stars.
Once planetary radio emission is detected, it will provide a unique approach for learning about the planetary spin, magnetic field, presence of satellites, and potentially the stellar wind properties.%
}{%
Once planetary radio emission is detected, it in general provides a unique approach for learning the planetary property. As the auroral radio flux is significantly modulated in association with several physical processes described in \S\ref{ss:timevariability}, the time variability is useful to imply the planetary spin rotation period or the presence of satellites. If spectra are obtained, the upper cut-off frequency can tell us the magnetic field strength of the planetary surfaces, which, when combined with constrains on the planetary mass via e.g. radial-velocity observations, tests  the proposed scaling law of planetary dynamo. 
Furthermore, the lower cut-off frequency of the RGHJ emission, which presumably corresponds to in situ plasma frequency, could probe the stellar wind property at the planetary orbit. In addition, the planetary radio power in such an extreme environment as RG's massive stellar wind will be valuable data points to test the empirical ``radiometric Bode's law''. %
...
\end{eqnarray}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
where $\sigma $ is the cross section of hydrogen atoms, assumed to be equal to Bohr radius here \memoYF{OK??}. 
...
\revise{One might expect}{Given that the density of stellar wind of evolved stars is fairly high, one might anticipate }
...
\end{equation}
%%%
where $P_{\rm min}(M_p)$ is the minimum orbital period to have cyclotron frequency larger than the ambient plasma frequency, while $P_{\rm max}(M_p)$ is the maximum orbital period given planetary mass to emit detectable radio flux. For the former, combining equations (\ref{eq:fcyc}) and (\ref{eq:fplasma_RG}) leads to
%%%
\begin{eqnarray}
a_{\rm min} (M_p) &=& 2.15~{\rm AU} \left( \frac{M_p}{M_{\rm J}} \right)^{-1.04} \left( \frac{t}{4.5~{\rm Gyr}} \right)^{0.43}  \\
&&\times \left( \frac{\dot M_{\star }}{10^{-8}M_{\odot}/{\rm yr}} \right)^{1/2} 
\end{eqnarray}
%%%
while for the latter, equations (\ref{eq:scalingB}) and (\ref
{eq:F_nu_RGHJ}) indicate
%%%
\begin{eqnarray}
a_{\rm max} (M_p) &=& 3.80~{\rm AU} \left( \frac{F_{\rm min}}{10~\mu {\rm Jy}} \right)^
