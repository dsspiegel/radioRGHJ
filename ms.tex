...
\revise{In reality, the temperature and luminosity based on the Bondi--Hoyle accretion (equations (\ref{eq:Lacc} and (\ref{eq:Tacc})) describes the situation only approximately. Precisely, only the neutral portion of the stellar wind can accrete onto the planet without interacting with planetary magnetic field, and the ionized portion would lose some energy at the bow shock before it accretes. On the other hand, the ionized plasmas interacting with magnetic field could have a cross section larger than the Bondi--Hoyle accretion radius. The detailed electromagnetic structure around RGHJs therefore requires elaborate numerical simulations, which is beyond the scope of this paper. Noting these uncertainties, in this paper we simply aim to give an order-of-magnitude estimate. }
...
\revise{We note that their data do not unambiguously distinguish RG from early AGB stars and thus the list probably consists of about 40\% early AGB stars. }\memoYF{grammatical question: doesn't ``consists of'' sound like ``A is made of B''?}
...
YF is supported from the Grant-in-Aid No. 25887024 by the Japan Society for the Promotion of Science and from an appointment to the NASA Postdoctoral
Program at NASA Ames Research Center, administered by Oak Ridge Affiliated Universities. 
...
\end{equation}
%%%
where $P_{\rm min}(M_p)$ is the minimum orbital period to have cyclotron frequency larger than the ambient plasma frequency, while $P_{\rm max}(M_p)$ is the maximum orbital period given planetary mass to emit detectable radio flux. For the former, combining equations (\ref{eq:fcyc}), (\ref{eq:scalingB}), and (\ref{eq:fplasma_RG}) leads to
%%%
\begin{eqnarray}
