...
In this section, we consider the number density of observed flux of the targets in a spherical shell of radius $r$. 
...
The probability that the maximum flux lies in the range between $F_{\rm max}$ and $F_{\rm max} + dF_{\rm max}$, denoted $\mathcal{P} [F_{\rm max}] dF_{\rm max}$, is the joint probability that one of the targets has flux between $F_{\rm max}$ and $F_{\rm max} + dF_{\rm max}$ and that all others have smaller flux.
...
The expected value for the maximum flux among the targets within a spherical shell at radius $r$ (with width $dr$), $\langle F_{\rm max} \rangle$, is obtained by calculating the flowing integral. 
%%%
\begin{eqnarray}
\langle F_{\rm max} \rangle &=& \int dF_{\rm max} \, F_{\rm max} \mathcal{P} [F_{\rm max}] \\
&=& n_r \int dF_{\rm max} F_{\rm max} P [< F_{\rm max}]^{n_r-1}  P[F_{\rm max}]
\end{eqnarray}
%%%

Figure \ref{fig:expectedFmax} shows $\langle F_{\rm max} \rangle$ as a function of distance $r$ with varying number density of the targets. 
